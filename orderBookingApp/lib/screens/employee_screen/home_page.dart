...
class _HomePageState extends ConsumerState<HomePage>
  ...
  final efficiencyProvider = Provider<double>((ref) {
    ...
    final ordersList =
        ordersState.orders?.maybeWhen(data: (list) => list, orElse: () => []) ??
        [];

    final totalShops = ordersList.map((order) => order.shopId).toSet().length;

    final visitedToday =
        ref
            .watch(ordersViewModelProvider)
            .orders
            ?.maybeWhen(
              data: (list) {
                ...
                return todayOrders.map((order) => order.shopId).toSet().length;
              },
              orElse: () => 0,
            ) ??
        0;
