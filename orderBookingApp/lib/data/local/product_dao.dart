...
class ProductDao {
  final _uuid = Uuid();
Future<void> insertProducts(List<Product> products, {bool markSynced = true}) async {
  ...
  Future<List<Product>> getAllProducts() async {
    ...
    for (final row in rows) {
      ...
      final subtypes = await getProductSubTypes(localId, serverProductId: serverProductId);
      ...
      result.add(Product(
        ...
        totalPrice: row['total_price'] != null ? (row['total_price'] as num).toDouble() : null,
  ...
  Future<List<ProductSubType>> getProductSubTypes(String? productLocalId, {int? serverProductId}) async {
    ...
    final whereClause = serverProductId != null
        ...
        : 'product_local_id = ?';
    final whereArgs = serverProductId != null ? [productLocalId, serverProductId] : [productLocalId];

    final rows = await db.query('product_subtypes', where: whereClause, whereArgs: whereArgs);

    return rows.map((row) {
      return ProductSubType(
        subItemId: row['sub_item_id'] as int?,
        measuringUnit: row['measuring_unit'] as String?,
        availableUnit: row['available_unit'] != null ? (row['available_unit'] as num).toDouble() : null,
  ...
  Future<void> deleteProductSubType(String localId) async {
    final db = await AppDatabase.database;
    await db.delete('product_subtypes', where: 'local_id = ?', whereArgs: [localId]);
  }
  /// Get product by ID
Future<Product?> getProductById(int productId) async {
  final db = await AppDatabase.database;
  final rows = await db.query('products', where: 'product_id = ?', whereArgs: [productId]);
  if (rows.isEmpty) return null;

  final row = rows.first;
  final localId = row['local_id'] as String?;
  final subtypes = await getProductSubTypes(localId, serverProductId: productId);
